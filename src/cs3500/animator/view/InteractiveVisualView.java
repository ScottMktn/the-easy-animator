...
public class InteractiveVisualView extends JFrame implements IInteractiveView, ActionListener {
  ...
  private JComboBox listOfLayers;
  ...
  public InteractiveVisualView(int tempo) {
    ...
    this.listOfLayers = new JComboBox();
  ...
  public void setListener(ActionListener listener) {
    ...
    this.listOfLayers.addActionListener(listener);
    this.visibleCheckbox.addActionListener(listener);
  }
   ...
   */
  private void initAddShapeObjects() {
  ...
  public int getSelectedLayer() {
    return this.listOfLayers.getSelectedIndex();
  }
  ...
  public void setVisibleIndicator(boolean value) {
    this.visibleCheckbox.setSelected(value);
  }
   ...
   */
  private JPanel buildKeyframeEditPanel() {
    JPanel topPanel = new JPanel(new FlowLayout());
  ...
  /**
   * Constructs the layer panel for the third tab on the view.
   *
   * @return the panel with the layer options.
   */
  private JPanel buildLayerEditPanel() {
    JPanel topPanel = new JPanel(new FlowLayout());
    JPanel layerFeatures = new JPanel(new GridLayout(6, 2));
    layerFeatures.setPreferredSize(this.getGridDimension());

    this.listOfLayers.setActionCommand(Command.SET_LAYER.getString());
    this.visibleCheckbox.setActionCommand(Command.SET_VISIBLE.getString());

    // Row 1)
    layerFeatures.add(new JLabel("Selected Layer: "));
    layerFeatures.add(this.listOfLayers);

    // Row 2)
    layerFeatures.add(new JLabel("Set Visible: "));
    layerFeatures.add(this.visibleCheckbox);

    // Row 3)
    layerFeatures.add(new JLabel(""));
    layerFeatures.add(this.buttons.get(Command.ADD_LAYER));

    // Row 4)
    layerFeatures.add(new JLabel(""));
    layerFeatures.add(this.buttons.get(Command.DELETE_LAYER));

    // Row 5)
    layerFeatures.add(new JLabel(""));
    layerFeatures.add(this.buttons.get(Command.MOVE_LAYER_UP));

    // Row 6)
    layerFeatures.add(new JLabel(""));
    layerFeatures.add(this.buttons.get(Command.MOVE_LAYER_DOWN));


    for (int i = 0; i < this.model.layerCount(); i++) {
      this.listOfLayers.addItem("" + i);
    }

    topPanel.add(layerFeatures);
    return topPanel;
  }
  ...
  private void initTabEditingPanel() {
    ...
    this.tabbedPane.add(this.buildLayerEditPanel(), "Layers");
    this.add(this.tabbedPane, BorderLayout.EAST);
    this.tabbedPane.addChangeListener(e -> this.updateComboBox());
  }
  ...
  public void updateComboBox() {
    ...
    int temp = this.listOfLayers.getSelectedIndex();
    try {
      this.listOfLayers.removeAllItems();
      for (int i = 0; i < this.model.layerCount(); i++) {
        this.listOfLayers.addItem("" + i);
      }
      this.listOfLayers.setSelectedIndex(temp);
      this.listOfShapes.removeAllItems();
      for (String s : initListOfShapesArray(model)) {
        this.listOfShapes.addItem(s);
      }
    } catch (Exception e) {
      this.showDialogBox(e.getMessage());
    }
  ...
  /**
   * Gets a change listener object that acts as the callback for when the slider is changed. Updates
   * the AnimationPanel's current tick as well as repaints.
   *
   * @return a ChangeListener for moving the scrubber.
   */
  private ChangeListener moveScrubber() {
    return e -> {
      this.animationPanel.setCurTick(this.scrubber.getValue());
      this.animationPanel.repaint();
      this.scrubber.setMaximum(this.animationPanel.getMaxTick());
    };
  }
  ...
  private void initAnimationPanel() {
    ...
    this.scrubber = new JSlider(JSlider.HORIZONTAL, 0,
            this.animationPanel.getMaxTick(), this.animationPanel.getCurTick());

    this.scrubber.addChangeListener(this.moveScrubber());
    Box scrubBox = Box.createVerticalBox();
    scrubBox.add(this.animationPanel);
    scrubBox.add(this.scrubber);
    this.add(new JScrollPane(scrubBox), BorderLayout.CENTER);
  }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    this.scrubber.setValue(this.animationPanel.getCurTick());
  }
}
